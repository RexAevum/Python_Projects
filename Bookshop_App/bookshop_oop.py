...
def search():
    ...
    results = db.search(nameValue.get(), authorValue.get(), yearValue.get(), isbnValue.get())
    for x in results:
        resultList.insert(END, x)

def insert():
    status = db.insert(nameValue.get(), authorValue.get(), yearValue.get(), isbnValue.get())
    if status:
        view_all()
        nameEntry.delete(0, END)
        authorEntry.delete(0, END)
...
def update():
    ...
    id = itemResult[0]
    status = db.update(id, nameValue.get(), authorValue.get(), yearEntry.get(), isbnEntry.get())
    if status:
...
def delete():
    ...
    status = db.delete(item[0])
    """
    status = db.delete(itemResult[0])
    if status:
